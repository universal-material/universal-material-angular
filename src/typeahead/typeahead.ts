...
import {TypeaheadWindow, ResultTemplateContext} from './typeahead-window';

/**
 * An event emitted right before an item is selected from the result list.
 */
export interface TypeaheadSelectItemEvent<T = any> {
  /**
...
export class Typeahead implements ControlValueAccessor,
  ...
  private _windowRef: ComponentRef<TypeaheadWindow>| null = null;
   ...
   */
  @Input() uTypeahead: OperatorFunction<string, readonly any[]>| null | undefined;
   ...
   *
   * Accepts a string containing CSS class to be applied on the `typeahead-window`.
   *
  ...
  */
  ...
  popupId = `typeahead-${nextWindowId++}`;
  ...
  constructor(
    ...
    config: TypeaheadConfig, ngZone: NgZone, private _live: Live, @Inject(DOCUMENT) private _document: any,
    ...
    this._popupService = new PopupService<TypeaheadWindow>(
      TypeaheadWindow, injector, viewContainerRef, _renderer, this._ngZone, componentFactoryResolver,
      applicationRef);
  ...
  ngOnChanges({typeahead}: SimpleChanges): void {
    if (typeahead && !typeahead.firstChange) {
  ...
  private _subscribeToUserInput(): void {
    const results$ = this._valueChanges.pipe(
      ...
      this.uTypeahead ? this.uTypeahead : () => of([]));
