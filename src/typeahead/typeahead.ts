...
/**
 * Typeahead directive provides a simple way of creating powerful typeaheads from any text input
 */
@Directive({
...
export class Typeahead implements ControlValueAccessor,
   ...
   */
  @Input() uTypeahead: (text: Observable<string>) => Observable<any[]>;
  ...
  */
  ...
  /**
   * An event emitted when a match is selected. Event payload is of type TypeaheadSelectItemEvent.
   */
  ...
  constructor(
      ...
      config: TypeaheadConfig, ngZone: NgZone, private _live: Live) {
    ...
    this._popupService = new PopupService<TypeaheadWindow>(
        TypeaheadWindow, _injector, _viewContainerRef, _renderer, componentFactoryResolver);
  ...
  ngOnInit(): void {
    ...
    const results$ = inputValues$.pipe(this.uTypeahead);
